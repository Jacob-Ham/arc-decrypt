...
When Azure Arc is deployed via Group Policy Object (GPO), the service principal secret is often encrypted and stored on a SYSVOL or network share. This tool automatically locates the deployment share and uses DPAPI-NG (NCryptProtectSecret) to unwrap the secret.
...
```bash
python3 arc-decrypt.py decrypt -d contoso.local -dc-ip 10.0.0.5 -u "WS01$" -p "MachinePass123"
```
